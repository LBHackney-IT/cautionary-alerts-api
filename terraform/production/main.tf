...
data "aws_ssm_parameter" "housing_prod_account_id" {
  name = "/housing-prod/account-id"
}
...
data "aws_iam_policy_document" "sns_topic_policy" {
  ...
  statement {
      ...
      principals {
        type        = "AWS"
        identifiers = ["arn:aws:iam::${data.aws_ssm_parameter.housing_prod_account_id.value}:role/LBH_Circle_CI_Deployment_Role"]
      }
      resources = [
        aws_sns_topic.cautionaryalerts_topic.arn
      ]
